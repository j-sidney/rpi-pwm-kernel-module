...
static int s_TogglePeriod = 500;
static const int LedGpioPin = 2;
...
static void TimerHandler(struct timer_list *unused)
{
    ...
    mod_timer(&s_Timer, jiffies + msecs_to_jiffies(s_TogglePeriod));
}
...
static int __init PwmModule_init(void)
{
    ...
    timer_setup(&s_Timer, TimerHandler, 0);
    result = mod_timer(&s_Timer, jiffies + msecs_to_jiffies(s_TogglePeriod));
    BUG_ON(result < 0);
